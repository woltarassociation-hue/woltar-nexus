...
import NotificationsPanel from "../components/account/NotificationsPanel.jsx";
    ...
    return (
      <div className="setup-page">
        <div className="setup-card">
          ...
          <Link to="/login" className="setup-btn">
            Se connecter
          </Link>
        </div>
      </div>
...
function AccountView({ user, profile, navigate }) {
  const [displayName, setDisplayName] = useState(
    profile?.displayName || profile?.display_name || profile?.name || profile?.username || ""
  );
  ...
  const [userBadges, setUserBadges] = useState(() =>
    profile?.id ? getProfileUserBadges(profile.id) : []
  );
  ...
  return (
    <div className="setup-page">
      <div className="setup-card account-card">
        ...
        <div className="account-role">{getRoleLabel(profile?.role)}</div>
        ...
        <div className="setup-form account-form">
          <div className="setup-field">
            ...
            <input className="setup-input" type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)} />
          </div>
          <div className="setup-field">
            <label className="setup-label">Woltarien(ne) principal(e)</label>
            <input className="setup-input" type="text" value={woltarien1} onChange={(e) => setWoltarien1(e.target.value)} />
          </div>
          <div className="setup-field">
            <label className="setup-label">Bio</label>
            <textarea className="setup-input" value={bio} onChange={(e) => setBio(e.target.value)} rows={4} />
          </div>
          <div className="setup-field">
          ...
          <div className="setup-field">
            <label className="setup-label">Woltarien(ne) secondaire</label>
            <input className="setup-input" type="text" value={woltarien2} onChange={(e) => setWoltarien2(e.target.value)} />
          </div>
        ...
        <NotificationsPanel profileId={profile?.id} />
      </div>
    </div>
  );
}
