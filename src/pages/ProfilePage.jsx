...
export default function ProfilePage() {
  ...
  // Regrouper loading / notFound / profile dans un seul objet pour éviter
  // les setState synchrones dans l'effet (règle react-hooks/set-state-in-effect).
  const [ps, setPs] = useState({ loading: true, notFound: false, profile: null });

  // Setter-only pour forcer un re-render quand le cache badges change.
  ...
  // Badges dérivés au render — recalculés à chaque changement de badgesV
      ...
      <div className="pubprofile-page">
        ...
        {!loading && notFound && (
          <div className="pubprofile-notfound">
            ...
            <p>L'utilisateur <strong>@{username}</strong> n'existe pas ou n'est pas accessible.</p>
            <Link to="/" className="pubprofile-back-btn">← Retour à l'accueil</Link>
          </div>
        )}
        ...
        {!loading && profile && (
          <div className="pubprofile-card">
                ...
                {visibleBadges.map((b) => {
                  ...
                  return (
                    <span
                      ...
                      title={[b.description, b.context ? `(${b.context})` : ""].filter(Boolean).join(" ")}
                    >
            ...
            {/* Bio */}
            {profile.bio && (
              <p className="pubprofile-bio">{profile.bio}</p>
            )}
            ...
            {(profile.bio || profile.woltarien1 || profile.links?.website) && (
              <hr className="pubprofile-divider" />
            )}

            {/* Statistiques — placeholders future-ready */}
            <div className="pubprofile-stats">
              ...
              <div className="pubprofile-stat">
                <strong>{visibleBadges.length || "—"}</strong>
                <span>Badges</span>
              </div>
            </div>
            ...
            <Link to="/" className="pubprofile-back-link">← Retour à l'accueil</Link>
          </div>
        )}
